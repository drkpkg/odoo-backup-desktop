# Contrato del módulo Odoo `appex_backup` (API v1)

Este documento define la API que el futuro módulo `appex_backup` (Odoo 15.0–19.0) debe exponer.
El cliente de escritorio (`crates/appex-odoo`, `AppexModuleTransport`) ya lo implementa.
...
- **No usa la contraseña maestra.** Autentica con API key de un usuario técnico que pertenece al
  grupo `appex_backup.group_backup_operator`.
...
- Un solo job activo por base de datos. Archivos `0600` en `<data_dir>/appex_backup/`, TTL por
  defecto 24 h.

## Métodos RPC — modelo `appex.backup.api` (`AbstractModel`, métodos `@api.model`)

Invocables por XML-RPC (`execute_kw(db, uid, key, "appex.backup.api", <método>, [], kwargs)`)
y por JSON-2 (`POST /json/2/appex.backup.api/<método>` con kwargs en el cuerpo).
...
```
GET /appex_backup/download/<job_id>
