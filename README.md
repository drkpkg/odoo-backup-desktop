...
- Dos transportes de backup: **gestor de bases de datos** (`/web/database/backup`, requiere
  `list_db = True` y contraseña maestra) y **módulo `appex_backup`** (funciona con `list_db = False`,
  usa API key; el módulo se desarrolla aparte, ver [docs/appex-backup-module-api.md](docs/appex-backup-module-api.md)).
...
o embébelos al compilar con `APPEX_GDRIVE_CLIENT_ID` y `APPEX_GDRIVE_CLIENT_SECRET`.

## Distribución

Los workflows de `.github/workflows/` generan los paquetes al crear un tag `v*`.
El PKGBUILD para Arch (AUR `appex-backup-bin`) está en `packaging/aur/`.
