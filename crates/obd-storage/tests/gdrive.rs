...
use md5::{Digest, Md5};
use obd_storage::gdrive::{DriveOptions, GoogleDriveAdapter, GoogleEndpoints, OAuthClient, RetryPolicy};
use obd_storage::{RemoteObject, StorageAdapter, StorageError, TargetId, TargetSpec, UploadMeta, UploadProgressFn};
...
fn about_ok() -> ResponseTemplate {
    ResponseTemplate::new(200).set_body_json(json!({"user": {"emailAddress": "ops@example.com", "displayName": "Ops"}}))
}
...
async fn mount_session(server: &MockServer, session: &str, total: usize) {
    Mock::given(method("POST"))
        ...
        .and(body_partial_json(json!({
            ...
            "parents": ["folder-1"],
            "appProperties": {"obdBackup": "file", "instanceId": "inst-1", "database": "cliente1"}
        })))
        .respond_with(
...
fn completed_with_md5(size: usize, md5: &str) -> ResponseTemplate {
    ResponseTemplate::new(200).set_body_json(json!({
        ...
        "md5Checksum": md5,
        "appProperties": {"obdBackup": "file", "instanceId": "inst-1"}
    }))
}
...
async fn ensure_target_reuses_root_and_creates_instance_folder() {
    ...
    Mock::given(method("GET"))
        ...
        .and(query_param_contains("q", "name = 'Odoo Backup Desktop'"))
        .and(query_param_contains("q", "appProperties has { key='obdBackup' and value='root' }"))
        .and(query_param("supportsAllDrives", "true"))
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_json(json!({"files": [{"id": "root-1", "name": "Odoo Backup Desktop"}]})),
        )
        .expect(1)
        .mount(&server)
        .await;
    ...
    Mock::given(method("POST"))
        ...
        .and(body_partial_json(json!({
            ...
            "parents": ["root-1"],
            "appProperties": {"obdBackup": "instance", "instanceId": "inst-9"}
        })))
...
async fn ensure_target_on_shared_drive_creates_missing_root() {
    ...
    Mock::given(method("POST"))
        .and(path("/drive/v3/files"))
        .and(body_partial_json(json!({"name": "Backups", "parents": ["sd-1"], "appProperties": {"obdBackup": "root"}})))
...
async fn list_backups_paginates() {
    ...
    Mock::given(method("GET"))
        ...
        .and(query_param_contains("q", "appProperties has { key='obdBackup' and value='file' }"))
