...
mod tests {
    ...
    fn maps_error_statuses() {
        ...
        let err = error_from_status(StatusCode::NOT_FOUND, &missing.to_string(), "appex.backup.api");
        assert!(err.is_model_not_found());

        let other_404 = error_from_status(StatusCode::NOT_FOUND, "<html>404</html>", "appex.backup.api");
