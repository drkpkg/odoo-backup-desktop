...
/// Model exposed by the `appex_backup` module.
pub const MODULE_API_MODEL: &str = "appex.backup.api";
...
pub struct ProbeReport {
    ...
    /// `appex.backup.api/get_info` answered with a compatible `api_version`.
...
pub async fn probe(http: &Client, input: ProbeInput) -> Result<ProbeReport> {
    ...
    let recommended_transport = if module.is_ok() && api_key {
        Some(TransportKind::AppexModule)
