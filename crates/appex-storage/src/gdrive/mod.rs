...
//! - Files/folders are tagged with `appProperties` (`appexBackup`, `instanceId`).
...
impl GoogleDriveAdapter {
    ...
    async fn root_folder(&self) -> Result<String> {
        ...
        let id = self
            .find_or_create_folder(&self.options.root_folder_name, &parent, true, &[("appexBackup", "root")])
            .await?;
...
impl StorageAdapter for GoogleDriveAdapter {
    ...
    async fn ensure_target(&self, spec: &TargetSpec) -> Result<TargetId> {
        ...
        let id = self
            .find_or_create_folder(
                ...
                &root,
                false,
                &[("appexBackup", "instance"), ("instanceId", spec.instance_id.as_str())],
            )
            .await?;
    ...
    async fn list_backups(&self, target: &TargetId) -> Result<Vec<RemoteObject>> {
        let query = format!(
            "'{}' in parents and trashed = false and appProperties has {{ key='appexBackup' and value='file' }}",
