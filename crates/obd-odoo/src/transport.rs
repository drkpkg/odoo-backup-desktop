...
//! - [`ObdModuleTransport`]: `obd_backup` module API (see `docs/obd-backup-module-api.md`).
...
impl ObdModuleTransport {
    ...
    async fn download(
        ...
        let url = endpoint(&self.base_url, &format!("obd_backup/download/{job_id}"))?;
        let mut state =
...
#[async_trait]
impl BackupTransport for ObdModuleTransport {
    fn kind(&self) -> TransportKind {
