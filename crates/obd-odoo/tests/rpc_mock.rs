...
async fn xmlrpc_maps_failed_login_and_missing_model() {
    ...
    Mock::given(method("POST"))
        .and(path("/xmlrpc/2/object"))
        .respond_with(xml_fault(2, "Object obd.backup.api doesn't exist"))
        .mount(&server)
        .await;
    let rpc =
        connect(common::client(), base(&server), "db1".into(), credentials(SecretKind::Password), RpcProtocol::XmlRpc);
    let err = rpc.call("obd.backup.api", "get_info", &[], Map::new()).await.unwrap_err();
...
async fn json2_sends_bearer_and_database_headers() {
    ...
    Mock::given(method("POST"))
        ...
        .await;
    Mock::given(method("POST"))
        .and(path("/json/2/obd.backup.api/get_info"))
    ...
    assert!(rpc.call("obd.backup.api", "get_info", &[], Map::new()).await.unwrap_err().is_model_not_found());
...
async fn probe_reports_module_and_db_manager() {
    ...
    assert_eq!(report.recommended_transport, Some(TransportKind::ObdModule));
