...
use obd_odoo::{
    BackupPhase, BackupRequest, BackupTransport, DbManagerTransport, ObdModuleTransport, OdooError, OdooRpc,
...
/// In-memory `obd.backup.api` implementation.
struct FakeModule {
    api_version: u64,
...
impl OdooRpc for FakeModule {
    ...
    async fn authenticate(&self) -> obd_odoo::Result<i64> {
        Ok(2)
    }
    ...
    ) -> obd_odoo::Result<Value> {
        assert_eq!(model, "obd.backup.api");
        self.calls.lock().unwrap().push(method.to_owned());
        if self.missing {
            return Err(OdooError::Rpc {
                code: obd_odoo::MODEL_NOT_FOUND.into(),
                message: "Object obd.backup.api doesn't exist".into(),
            });
        }
        match method {
            "get_info" => Ok(
...
fn module_transport(rpc: Arc<FakeModule>, server: &MockServer) -> ObdModuleTransport {
    ObdModuleTransport::new(
        rpc,
        common::client(),
        Url::parse(&server.uri()).unwrap(),
        "api-key".into(),
...
async fn module_happy_path() {
    ...
    Mock::given(method("GET"))
        .and(path("/obd_backup/download/job-1"))
