...
fn password() -> Credentials {
    Credentials {
        login: env("OBD_IT_ADMIN_LOGIN"),
        secret: env("OBD_IT_ADMIN_PASSWORD").into(),
...
fn api_key() -> Credentials {
    Credentials {
        login: env("OBD_IT_ADMIN_LOGIN"),
        secret: env("OBD_IT_ODOO19_API_KEY").into(),
        kind: SecretKind::ApiKey,
    }
}

fn progress() -> obd_odoo::ProgressFn {
    std::sync::Arc::new(|_| {})
}
...
async fn it_detects_versions() {
    let http = client();
    for (var, major) in [("OBD_IT_ODOO15_URL", 15), ("OBD_IT_ODOO17_URL", 17), ("OBD_IT_ODOO19_URL", 19)] {
...
async fn it_xmlrpc_with_password_on_15_and_17() {
    let http = client();
    for (var, db) in [("OBD_IT_ODOO15_URL", "it15"), ("OBD_IT_ODOO17_URL", "it17")] {
        ...
        let missing = rpc.call("obd.backup.api", "get_info", &[], Map::new()).await.unwrap_err();
...
async fn it_json2_and_xmlrpc_with_api_key_on_19() {
    let http = client();
    let base = url("OBD_IT_ODOO19_URL");
    ...
    assert_eq!(users[0]["login"], "admin");
    let missing = json2.call("obd.backup.api", "get_info", &[], Map::new()).await.unwrap_err();
...
async fn it_probe_reports_module_missing_and_db_manager_ok() {
    let http = client();
    let cases = [
        ("OBD_IT_ODOO15_URL", "it15", password(), RpcProtocol::XmlRpc),
        ("OBD_IT_ODOO17_URL", "it17", password(), RpcProtocol::XmlRpc),
        ("OBD_IT_ODOO19_URL", "it19", api_key(), RpcProtocol::Json2),
    ];
    for (var, db, credentials, expected_protocol) in cases {
        let report = probe(
            ...
            ProbeInput {
                ...
                master_password: Some(env("OBD_IT_MASTER_PASSWORD").into()),
...
async fn backup_with_db_manager(
    ...
    let transport =
        DbManagerTransport::new(http, base, env("OBD_IT_MASTER_PASSWORD").into(), version, Duration::from_secs(600));
...
async fn it_db_manager_backup_on_all_versions() {
    ...
    let b15 = backup_with_db_manager("OBD_IT_ODOO15_URL", "it15", 15, true).await;
    assert!(b15.manifest.filestore_files > 0);
    let b17 = backup_with_db_manager("OBD_IT_ODOO17_URL", "it17", 17, true).await;
    assert!(b17.manifest.filestore_files > 0);
    let b19 = backup_with_db_manager("OBD_IT_ODOO19_URL", "it19", 19, true).await;
    assert!(b19.manifest.filestore_files > 0);
    let b19_no_fs = backup_with_db_manager("OBD_IT_ODOO19_URL", "it19", 19, false).await;
...
async fn it_db_manager_wrong_master_password() {
    let http = client();
    for (var, db) in [("OBD_IT_ODOO15_URL", "it15"), ("OBD_IT_ODOO19_URL", "it19")] {
        let base = url(var);
        let version = detect_version(&http, &base).await.unwrap();
        let dir = tempfile::tempdir().unwrap();
        let request = BackupRequest {
...
async fn it_db_manager_unknown_database() {
    let http = client();
    for var in ["OBD_IT_ODOO15_URL", "OBD_IT_ODOO19_URL"] {
        let base = url(var);
        let version = detect_version(&http, &base).await.unwrap();
        let dir = tempfile::tempdir().unwrap();
        let request = BackupRequest {
        ...
        let transport = DbManagerTransport::new(
            http.clone(),
            base,
            env("OBD_IT_MASTER_PASSWORD").into(),
            version,
...
async fn it_list_db_disabled() {
    ...
    for (var, db, credentials) in
        [("OBD_IT_ODOO15_NOLIST_URL", "it15", password()), ("OBD_IT_ODOO19_NOLIST_URL", "it19", api_key())]
    {
        let base = url(var);
        let report = probe(
            &http,
